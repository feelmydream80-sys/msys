...
def send_email(to, subject, html_content):
    ...
    try:
        ...
        error_msg = f"SMTP 연결 실패: 서버에 연결할 수 없습니다. ({e})"
        logger.error(f"[오류] {error_msg}")
        return False, error_msg
    except smtplib.SMTPHeloError as e:
        error_msg = f"SMTP HELO 실패: 서버가 HELO 명령을 거부했습니다. ({e})"
        logger.error(f"[오류] {error_msg}")
        return False, error_msg
    except smtplib.SMTPSenderRefused as e:
        error_msg = f"발신자 거부됨: 서버가 발신자 주소를 거부했습니다. ({e})"
        logger.error(f"[오류] {error_msg}")
        return False, error_msg
    except smtplib.SMTPRecipientsRefused as e:
        error_msg = f"수신자 거부됨: 서버가 수신자 주소를 거부했습니다. ({e})"
        logger.error(f"[오류] {error_msg}")
        return False, error_msg
    except smtplib.SMTPDataError as e:
        error_msg = f"SMTP 데이터 오류: 메일 데이터 전송 중 오류 발생 ({e})"
        logger.error(f"[오류] {error_msg}")
        return False, error_msg
    except smtplib.SMTPException as e:
        error_msg = f"SMTP 일반 오류: {e}"
        logger.error(f"[오류] {error_msg}")
        return False, error_msg
    except ConnectionRefusedError as e:
        error_msg = f"연결 거부됨: 서버에서 연결을 거부했습니다. ({e})"
        logger.error(f"[오류] {error_msg}")
        return False, error_msg
    except TimeoutError as e:
        error_msg = f"연결 타임아웃: 서버에 연결하지 못했습니다. ({e})"
        logger.error(f"[오류] {error_msg}")
        return False, error_msg
